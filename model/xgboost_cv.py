...
cv_dir = os.path.join(output_path, "xgboost", "cross_validation")
...
def train_single_xgboost_model(X, y, lower_bounds, upper_bounds, params, X_val=None, y_val=None, lower_bounds_val=None, upper_bounds_val=None, train_idx=None, val_idx=None, save_model_fName=None, verbose=False):
    ...
    for n in np.arange(10, max_num_estimators+1, step_size):
        
        model = xgb.XGBRegressor(**params,
                                 n_estimators=n,
                                 max_depth=5, # use this for all models
                                 objective=loss_func,
    ...
    # return lists of the training and validation losses and the minimum loss
    return train_loss_array, val_loss_array, min_loss
...
for params in ParameterGrid(param_grid):
    ...
    for split, (train_idx, val_idx) in enumerate(kfold_splits.split(np.zeros(len(df_train_val)), df_train_val["Binary"])): 
        ...
        _, _, min_loss = train_single_xgboost_model(X_train, 
                                                    y_train,
                                                    lower_bounds_train,
                                                    upper_bounds_train,
                                                    params,
        ...
        print(f"    Minimum validation loss for split {split+1}: {min_loss}")
        

pd.concat(losses_df).to_csv(os.path.join(f"{xgboost_dir}/losses_df.csv"), index=False)

# read in the losses dataframe
losses_df = pd.read_csv(os.path.join(f"{xgboost_dir}/losses_df.csv"))

best_params = pd.DataFrame(losses_df.groupby(['learning_rate', 'reg_lambda'])['val_loss'].mean()).reset_index().sort_values("val_loss", ascending=False).iloc[0, :]
best_params = dict(best_params)
del best_params['val_loss']

print(f"Performing cross-validaiton with best parameters: {best_params}\n")
...
for split, (train_idx, val_idx) in enumerate(kfold_splits.split(np.zeros(len(df_train_val)), df_train_val["Binary"])): 
    ...
    train_loss_array, val_loss_array, min_loss = train_single_xgboost_model(X_train, 
                                                                            y_train,
                                                                            lower_bounds_train,
                                                                            upper_bounds_train,
                                                                            best_params,
    ...
    print(f"    Minimum loss for split {split+1}: {min_loss}")
...
# save
pd.concat(cv_model_results).to_csv(f"{xgboost_dir}/results.csv")
