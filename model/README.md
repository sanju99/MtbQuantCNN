...
4. <code>cnn_custom_loss.py</code>: Trains a quantitative CNN (with or without early stopping) using the custom bounded loss function.
5. <code>cnn_standard.py</code>: Trains a quantitative or binary CNN (with or without early stopping) using off-the-shelf TensorFlow loss functions.
...
Early stopping stops the model training when the loss has not decreased for a user-specified number of epochs. 
