...
intermediate_df = f"{out_dir}/data_intermediate_clean.csv"

if len(who_variants.query("drug==@drug & confidence=='1) Assoc w R'")) == 0:
    
    print(f"There are no Tier 1 variants for {drug}")
    search_who_variants = False

else:

    search_who_variants = True
    isolate_variants_fName = "/n/data1/hms/dbmi/farhat/Sanjana/MIC_data/single_drugs/isolate_variants_fixed_annot.csv"
    
    if not os.path.isfile(isolate_variants_fName):
        raise ValueError(f"{isolate_variants_fName} does not exist. Please create it!")
    else:
        isolate_variants = pd.read_csv(isolate_variants_fName)
...
# lineages file should already be cleaned by script #3
...
df_combined.to_csv(intermediate_df, index=False)