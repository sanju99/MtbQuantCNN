...
all_isolates = np.concatenate([df_train.ROLLINGDB_ID.values, df_val.ROLLINGDB_ID.values])

with open(os.path.join(data_dir, drug, "isolates.txt"), "w") as file:
    for isolate in all_isolates:
        file.write(isolate + "\n")

print(f"Extracting variants for {len(all_isolates)} isolates")

# some of the IDs that are numbers are saved as integers, not strings, so specify the dtype for the Isolate column
train_isolate_variants = pd.read_csv(f"{data_dir}/trainVal_isolateVariants_AllDrugs.tsv", sep="\t", usecols=["Isolate"], dtype={"Isolate": str})
missing_isolates = set(all_isolates) - set(train_isolate_variants.Isolate)

if len(missing_isolates) > 0:
    print(f"{len(missing_isolates)} isolates are not in the isolate variants file")
...
if len(high_conf_genes) > 0:
    ...
    print(f"There are no genes with Category 1 {drug}-R-associated mutations!")
