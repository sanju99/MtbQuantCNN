...
##example command: perl snpConcatenater_w_exclusion_frompilonvcf.pl <file_w_VCF_paths.txt> <file_w_coord_start_stop_to_exclude in BED format> <failed_ID_list> [INDEL|SNP] [REGION|WHOLE] <startcoord-zerobased>-<endcoord-1based> <strand:pos or neg> > output_filename.fasta>\n REGION is optional; IF REGION is provided start stop and strand need to both be defined\n"

use warnings;
use strict;

if ($#ARGV < 1) {
    print STDERR "example command: perl snpConcatenater_w_exclusion_frompilonvcf.pl <file_w_VCF_paths.txt> <file_w_coord_start_stop_to_exclude in BED format> <failed_ID_list> [INDEL|SNP] REGION <startcoord-zerobased>-<endcoord-1based> <strand:pos or neg> > output_filename.fasta>\n REGION is optional; IF REGION is provided start stop and strand need to both be defined\n";
    die;
}
...
my @fileListRaw =&ReadInFile("$pathsFile");
my @fileList;
...
if ($option4 =~ m/REGION/i ) {
    ...
    $strand= shift(@ARGV);
    if ($regionstart !~ /\d+/) {
        print STDERR "If REGION is provided start and stop need to both be defined\nexample command: ./snpConcatenater_w_exclusion_frompilonvcf.pl <file_w_coord_start_stop_to_exclude in BED format> INDEL REGION <startcoord-zerobased>-<endcoord-1based> >output_filename.fasta>\n";
        die;
    }
}
...
for (my $i=0; $i<=$#fileList; $i++) {
    ...
    foreach my $line (@fileRaw) {
       ...
       # if ($line =~ /^NC_000962\.3/) {
        ...
        # insertions, deletions, and multi-base substitutions
        if (length($ref_allele) > 1 || length($allele) > 1) {
            ...
            if (length($ref_allele) == length($allele)) {
                ...
                for (my $i=0; $i<length($ref_allele); $i++){
                    ...
                    # if the variant did not pass quality-control assign to N
                    if ($ex > 0) {
                        $masterHash{$strainName}{$pos} = "N" x length($base);
                    }
                    else {
            ...
            # if this == SNP, then don't process indels 
            if ($option3 =~ m/INDEL/i) {
            
                # insertions and deletions where one is a single nucleotide
                if (length($ref_allele) == 1 || length($allele) == 1) {

                    # deletion case -- reference has nucleotide, others have gap character
                    if (length($ref_allele) > length($allele)) {
                        for (my $i=1; $i<length($ref_allele); $i++){ #ignoring the zero index base in $ref_allele and $allele
                            ...
                            if ($pos <= $regionend) {
                                ...
                                # if the variant did not pass quality-control assign to N. In this case, len(allele) = 1, so add just 1 N
                                if ($ex > 0) {
                                    $masterHash{$strainName}{$pos} = "N";
                                }
                                else {
                    ...
                    # insertion case
                        ...
                        for (my $i=1; $i<length($allele); $i++){ #ignoring the zero index base in $ref_allele and $allele
                            ...
                            # if the variant did not pass quality-control assign to N
                            if ($ex > 0) {
                                $insertionHash{$strainName}{$pos} = "N" x length($base);
                            }
                            else {
                ...
                # complex variants = REF and ALT are both multiple nucleotides long, and len(REF) != len(ALT)
                else {
        ...
        # single base substitution
            ...
            # if the variant did not pass quality-control assign to N
            if ($ex > 0) {
                $masterHash{$strainName}{$from} = "N" x length($allele);
            }
            else {
                $masterHash{$strainName}{$from} = $allele;
            }

            # update coordinates, regardless of the quality control value
...
my @uniqueCoords = sort {$a <=> $b} @allCoords;
@insCoords = sort {$a <=> $b} @insCoords;
...
if ($option4 =~ m/REGION/i) { #expand uniqueCoords to include all sites of the region, and place insertions in right place i.e. not at the end as above code does
    ...
    for (my $i=0; $i<=$#allStrains; $i++) {
        ...
        $counterXT=0; $counterIT=0; $last=0; $h=0;
        foreach my $pos (@insCoords) { #introduces gaps into the sequence in place of insertions in other strains
            if ($pos-$last >1) {
                $counterXT=$counterXT+$counterIT;
                $counterIT=1;
            }
