...
# INPUTS: 1) high-level directory where all samples are stored, and 2) array of sample_IDs
# this input comes from bash, which will be a newline-delimited array, which is then read as a string into Python, so convert
_, sample_IDs_fName, output_dir = sys.argv

sample_IDs = pd.read_csv(sample_IDs_fName, header=None, sep='\t')[0].unique()
print(f"Computing F2 metrics for {len(sample_IDs)} samples")
...
for i, sample_ID in enumerate(sample_IDs):
    ...
    sample_lineage_dir = os.path.join(output_dir, sample_ID, "lineage_calling")
    reduced_VCF = os.path.join(sample_lineage_dir, f"{sample_ID}_lineage_positions.vcf")
    F2_txt = os.path.join(sample_lineage_dir, f'{sample_ID}_F2_Coll2014.txt')
    ...
    if os.path.isfile(F2_txt):
        ...
        if os.path.isfile(reduced_VCF):
            ...
            os.remove(f"{sample_lineage_dir}/{sample_ID}.bcf.csi")
            os.remove(f"{sample_lineage_dir}/{sample_ID}_lineage_positions.vcf")
