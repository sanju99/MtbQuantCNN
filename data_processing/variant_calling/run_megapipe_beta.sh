...
#SBATCH -p short 
#SBATCH --mem=30G
#SBATCH -o ~/Errors/zerrors_%j.out 
#SBATCH -e ~/Errors/zerrors_%j.err 
...
# extract_kraken_reads_script="~/data_cleaning/extract_kraken_reads.py"
...
do
    ...
    if [ -f "$sample_pilon_dir/${sample_ID}_variants.vcf" ] && [ ! -f "$sample_pilon_dir/${sample_ID}_variants_combinedCodons.vcf" ]; then
        ...
        if [ ! -f "$sample_pilon_dir/${sample_ID}_variants_combinedCodons.vcf" ]; then        
            ...
            python3 -u ~/MtbQuantCNN/data_processing/variant_calling/combine_codon_variants.py "$sample_pilon_dir/${sample_ID}_variants.vcf"
        fi
