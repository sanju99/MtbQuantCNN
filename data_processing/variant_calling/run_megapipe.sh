...
# extract_kraken_reads_script="/home/sak0914/data_cleaning/extract_kraken_reads.py"
lineage_def_ref_pos="/home/sak0914/MtbQuantCNN/data_processing/variant_calling/Coll2014_positions_all.txt" # for F2 calculation
...
function align_reads_drop_duplicates() {
    ...
    if [ "$(awk 'BEGIN{print ('$unclassified_percent' <= 10)}')" -eq 1 ]; then        
    ...
    if [ ! -f "$sample_pilon_dir/${sample_ID}_full.vcf.gz" ] || [ ! -f "$sample_pilon_dir/${sample_ID}_variants.vcf" ]; then
        ...
        if [ $num_run_IDs -eq 0 ]; then
            ...
            if [ ! -f "$sample_bam_dir/pass_run_IDs.txt" ]; then
                python3 -u /home/sak0914/MtbQuantCNN/data_processing/variant_calling/BAM_depth_QC.py "$sample_out_dir"
            fi
            ...
            if [ $num_runs_passed -eq 0 ]; then
                ...
                if [ $num_runs_passed -eq 1 ]; then
                    ...
                    # this file was also already indexed in trim_classify_align.sh, so it doesn't need to be done again
...
python3 -u /home/sak0914/MtbQuantCNN/data_processing/variant_calling/F2_calculation.py $input_file $out_dir