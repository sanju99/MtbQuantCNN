...
do
    ...
    if [ ! -f "$sample_pilon_dir/${sample_ID}_full.vcf.gz" ] || [ ! -f "$sample_pilon_dir/${sample_ID}_variants.vcf" ]; then
        ...
        if [ $num_run_IDs -eq 0 ]; then
            ...
            num_runs_passed=$(wc -l < "$sample_bam_dir/pass_run_IDs.txt")
            ...
            if [ $num_runs_passed -eq 0 ]; then
                ...
                if [ $num_runs_passed -ne 0 ]; then
                    ...
                    if [ $num_runs_passed -eq 1 ]; then
                        ...
                        old_bam_file=$(cat "$sample_bam_dir/pass_run_IDs.txt")
    
                        # move the BAM file to the sample_bam_dir and rename it with the sample ID so that it's easier to find that having to go into run_bam_dir
                        # mv $old_bam_file $bam_file
