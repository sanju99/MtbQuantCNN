import numpy as np
import pandas as pd
import glob, os, sys, vcf, pickle, yaml, itertools, tracemalloc
import Bio
import Bio.SeqUtils
...
if os.path.isfile(os.path.join(data_dir, drug, "isolate_variants_high_conf.csv")):
    highConf_isolate_variants_df = pd.read_csv(os.path.join(data_dir, drug, "isolate_variants_high_conf.csv"), dtype={"Isolate": str})
else:
    print(f"There are no high confidence {drug}-R-associated mutations. This script is not needed")
    exit()
                ...
                if row["FILTER"] == "PASS" and len(row["REF"]) == 1 and len(row["ALT"]) == 1:
                    ...
                    if pos in pos_to_fix:
                        ...
                        if pos not in fixed_pos:
                            ...
                            if gene in h37Rv_genes_annot.Symbol.unique():
                            ...
                            print(f"{gene} is not in the Mycobrowser list of genes")
...
isolates_lst = highConf_isolate_variants_df['Isolate'].unique()

print(f"Fixing annotations for {highConf_isolate_variants_df.Isolate.nunique()} samples in the range [{START},{END}] and saving to {output_file}")
for i, sample_id in enumerate(isolates_lst):
    
    if len(highConf_isolate_variants_df.query("Isolate==@sample_id").loc[~pd.isnull(highConf_isolate_variants_df["POS"])]) == 0:

        nan_df = pd.DataFrame(pd.Series([np.nan]*(len(highConf_isolate_variants_df.columns)-1) + [sample_id])).T
        nan_df.columns = highConf_isolate_variants_df.columns
        df_new_lst.append(nan_df)

    else:
        df_new_lst.append(fix_mutations_single_sample(sample_id, highConf_isolate_variants_df))
    
    if i % int(len(isolates_lst) / 10) == 0:
        print(i)
        pd.concat(df_new_lst, axis=0).to_csv(output_file, index=False)

pd.concat(df_new_lst, axis=0).to_csv(output_file, compression="gzip", index=False)
