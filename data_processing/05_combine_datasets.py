...
validation_vcf_dir = "/n/scratch3/users/s/sak0914/MIC_ML"

# validation data for a single drug
df_train = pd.read_csv(os.path.join(out_dir, "data_for_model.csv"))
...
for sample_id in df_val["ROLLINGDB_ID"].values:
    ...
    #if os.path.isfile(os.path.join(validation_vcf_dir, sample_id, "kraken/kraken_report")):
    if os.path.isfile(os.path.join(validation_vcf_dir, sample_id, f"pilon/{sample_id}_full.vcf.gz")):
...
if not os.path.isfile(f"{out_dir}/validation_PASS_prop.txt"):
    print(f"Please create {out_dir}/validation_PASS_prop.txt before running this script! \n   Command:bash data_processing/QC_scripts/check_pass_proportion.sh {os.path.join(out_dir, 'validation_data.csv')} {out_dir}/validation_PASS_prop.txt {validation_vcf_dir} {START} {END}")
    raise ValueError()
...
validation_PASS_prop_df = get_samples_PASS_prop(f"{out_dir}/training_PASS_prop.txt")
...
# add VCF file paths to the validation data file???
...
df_train["Primary_Lineage"] = [val[0] if "." in val else val.replace("_", "") for val in df_train["Lineage"]]
stratify_vals = df_train["Primary_Lineage"] + "-" + df_train["Binary"].astype(str)
...
for lineage in summary_counts["Lineage"].unique():
    if len(summary_counts.query("Lineage == @lineage").Resistance.unique()) < 2:
        ...
        df_train = df_train.query("Primary_Lineage not in @lineage")
...
with open(os.path.join(out_dir, "combined_paths_for_aln.txt"), "w+") as file:
    ...
    for sample_id in df_val["ROLLINGDB_ID"].values:
        ...
        fName = os.path.join(validation_vcf_dir, sample_id, "pilon", f"{sample_id}.vcf")
        assert os.path.isfile(fName)
        file.write(fName + "\n")