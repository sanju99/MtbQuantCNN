...
test_generator = MtbGeneDataset(
    ...
    bounded_loss=False, # don't need bounded loss for this, so don't load the bounds in
...
# third dimension: CNN_inputs, can be of length 3-5. If length 3, there is nucleotide matrix, lower bounds, and upper bounds.
# but here, there are no bounds, so take the shape of the last element, which will be the MLP inputs
...
if include_lineage or include_peptide_lengths:
    additional_data_len = test_generator[0][0][-1].shape[1]
else:
...
if train_model:
    ...
    train_single_CNN(model, loss_type, N_epochs, train_generator, val_generator, len(df_train), len(df_val), save_model_fName=os.path.join(output_path, "best_model.h5"), save_history_df=True, patience_epochs=patience_epochs, return_min_loss=False)


# get predictions if the weights file exists
if os.path.isfile(os.path.join(output_path, "best_model.h5")):
    ...
    # don't need bounded loss for this, so don't load the bounds in
    if include_amino_acid_properties:
        best_model = multi_conv_nn(binary, longest_locus, num_loci, longest_protein, num_proteins, additional_data_len, False, filter_size, reg_strength=0)
    else:
        best_model = conv_nn(binary, longest_locus, num_loci, additional_data_len, False, filter_size, reg_strength=0)
    ...
    summary_df = create_summary_df(df_test,
                                   ...
                                   model_name="CNN", 
                                   binarize=True, 
                                   save_fName=test_predictions_fName,
                                  )
    summary_df["CV"] = 0
    ...
    summary_df.to_csv(test_results_fName, index=False)
    K.clear_session()
