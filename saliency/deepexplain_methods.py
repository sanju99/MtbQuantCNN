        ...
        if self.baseline is None:
            ...
            if self.has_multiple_inputs:
                for i, xi in enumerate(self.X):
                    if self.baseline[i].shape == xi.shape[1:]:
                        self.baseline[i] = np.expand_dims(self.baseline[i], 0)
                    else:
                        raise RuntimeError(f'Baseline shape {self.baseline[i].shape} does not match expected shape {xi.shape[1:]}')
            else:
                if self.baseline.shape == self.X.shape[1:]:
                    self.baseline = np.expand_dims(self.baseline, 0)
                else:
                    raise RuntimeError(f'Baseline shape {self.baseline.shape} does not match expected shape {self.X.shape[1:]}')
