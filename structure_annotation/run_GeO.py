...
NUM_SHUFFLES = int(kwargs["NUM_SHUFFLES"])
absolute_path = kwargs["absolute_path"]
prot_id = kwargs["prot_id"]
output_path = kwargs["output_path"]
mutations_file = kwargs["mutations_file"]

time0 = time.time()

# read the distance map
# DON'T INCLUDE .CSV OR .NPY IN THE FILE EXTENSION
dm = DistanceMap.from_file(f"{absolute_path}/{prot_id}")

# read the mutations to analyze
to_analyze = pd.read_csv(f"{absolute_path}/{output_path}/{mutations_file}")
...
result_table.to_csv(f"{absolute_path}/{output_path}/random_GeO_pvalues_{NUM_SHUFFLES}.csv.gz", compression='gzip', index=False)
...
print(f"shuffles accomplished in {time.time() - time1} seconds")