...
genotype_input_directory = f"{genotype_input_directory.replace('fastas', f'AF_thresh_{int(AF_thresh*100)}/fastas')}"
...
if not os.path.isfile(os.path.join(test_seq_data_path, "pkl_sparse_test.npz")):
...
if include_peptide_lengths and not os.path.isfile(os.path.join(test_seq_data_path, "gene_peptide_lengths.csv")):
    ...
    locus_peptide_lengths = make_CDS_length_df(drug, kwargs["tier1_loci"] + kwargs["tier2_loci"], genotype_input_directory, os.path.join(test_seq_data_path, "seqDict.pkl"))
    
    # keep index because that's the samples column
    locus_peptide_lengths.to_csv(os.path.join(test_seq_data_path, "gene_peptide_lengths.csv"))
...
if include_amino_acid_properties:
    ...
    if not os.path.isfile(os.path.join(test_seq_data_path, "pkl_AA_train_val.npy")) or not os.path.isfile(os.path.join(test_seq_data_path, "pkl_AA_test.npy")):
        ...
        create_AA_alns(drug, kwargs["tier1_loci"] + kwargs["tier2_loci"], genotype_input_directory, os.path.join(test_seq_data_path, "seqDict.pkl"))
...
print(f"Loading in model {os.path.join(output_path, 'best_model.h5')}")
...
summary_df = create_summary_df(df_test,
                               ...
                               model_name="CNN", 
                               binarize=True, 
                               save_fName=os.path.join(output_path, f"test_predictions_AF{int(AF_thresh*100)}.csv"),
                              )

summary_df.to_csv(os.path.join(output_path, f"full_model_results_AF{int(AF_thresh*100)}.csv"))
